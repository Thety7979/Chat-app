...
public interface CallDAO extends JpaRepository<Call, UUID> {
    ...
    List<Call> findByConversationIdAndStatusOrderByCreatedAtDesc(@Param("conversationId") UUID conversationId, @Param("status") Call.CallStatus status);
    
    @Query("SELECT c FROM Call c WHERE c.initiator.id = :userId AND c.status = :status ORDER BY c.createdAt DESC")
    List<Call> findByInitiatorIdAndStatusOrderByCreatedAtDesc(@Param("userId") UUID userId, @Param("status") Call.CallStatus status);
    ...
    List<Call> findByConversationIdAndTypeOrderByCreatedAtDesc(@Param("conversationId") UUID conversationId, @Param("type") Call.CallType type);
    
    @Query("SELECT c FROM Call c WHERE c.initiator.id = :userId AND c.type = :type ORDER BY c.createdAt DESC")
    List<Call> findByInitiatorIdAndTypeOrderByCreatedAtDesc(@Param("userId") UUID userId, @Param("type") Call.CallType type);
    
    @Query("SELECT COUNT(c) FROM Call c WHERE c.conversation.id = :conversationId AND c.status = :status")
    long countByConversationIdAndStatus(@Param("conversationId") UUID conversationId, @Param("status") Call.CallStatus status);
    ...
    List<Call> findExpiredCallsByStatus(@Param("status") Call.CallStatus status, @Param("expiredBefore") java.time.Instant expiredBefore);
}
